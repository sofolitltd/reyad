  ...
  return (
    <div className="bg-white rounded-xl shadow-2xl overflow-hidden transform hover:scale-105 transition-transform duration-300 ease-in-out flex flex-col h-full border border-gray-200">
      ...
      <div className="p-6 flex flex-col flex-grow">
        <h3 className="text-2xl font-semibold mb-3 text-teal-500">{project.title}</h3>
        <p className="text-gray-700 mb-4 text-sm leading-relaxed flex-grow">{project.description}</p>
        <div className="mb-4">
          {project.tags.map((tag) => (
            <span
              key={tag}
              className="inline-block bg-teal-50 border border-teal-100 rounded-full px-3 py-1 text-xs font-semibold text-teal-600 mr-2 mb-2"
            >
              {tag}
            </span>
          ))}
        ...
        <div className="mt-auto flex justify-start space-x-4 pt-2">
          {project.liveUrl && (
            <Link href={project.liveUrl} 
                ...
                className="inline-block bg-teal-500 hover:bg-teal-600 text-white font-medium py-2 px-4 rounded-lg text-sm transition duration-300"
              >
                Live Demo
