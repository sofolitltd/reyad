...
const structure = {
  ...
  children: [
    ...
    },
    {
      name: 'public',
      type: 'folder',
      children: [{ name: 'cv.pdf', type: 'file', id: 'download-cv', icon: Download, color: 'text-purple-400' }],
    },
  ],
};
...
export function ProjectExplorer({
        ...
        <div className="bg-sidebar border-t border-sidebar-border flex justify-around items-center h-16">
          {allFiles.map(file => (
            <Button
              ...
              variant="ghost"
              className={cn('h-full w-full flex-col gap-1 text-xs rounded-none', activeFile === file.id ? 'bg-background text-primary' : 'text-muted-foreground')}
            >
  ...
  return (
    <>
      <DownloadCvDialog open={isDialogOpen} onOpenChange={setIsDialogOpen} />
      <div className="p-2">
        <div className={cn('flex items-center justify-between', !isExpanded && 'justify-center')}>
          {isExpanded && (
